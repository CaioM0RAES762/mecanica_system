...
export default function CadastroPage() {
  ...
  const [perfil, setPerfil] = useState<'supervisor' | 'mecanico'>('mecanico')
  ...
  async function handleEtapa1(e: FormEvent) {
    ...
    const val = RegistrarSchema.safeParse({ nome_completo: nomeCompleto, cargo, perfil, email })
    if (!val.success) {
    ...
    try {
      ...
      if (res.status === 409) {
        setErro('Conta já cadastrada. Faça login ou recupere sua senha.')
        return
      }
      if (!res.ok) {
        setErro(data.detail ?? 'Erro ao processar o cadastro.')
        return
      }
  ...
  async function handleEtapa2(e: FormEvent) {
    ...
    const val = VerificarCodigoCadastroSchema.safeParse({ email, codigo })
    if (!val.success) {
      setErro(val.error.issues.map((i) => i.message).join(' · '))
      return
    }
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API_URL}/auth/verificar-codigo-cadastro`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(val.data),
      })
      const data = (await res.json()) as { valido?: boolean; detail?: string }

      if (!res.ok) {
        setErro(data.detail ?? 'Código inválido ou expirado.')
        return
      }
  ...
  async function handleEtapa3(e: FormEvent) {
    ...
    const val = FinalizarCadastroSchema.safeParse({ email, codigo, senha, confirmar_senha: confirmarSenha })
    if (!val.success) {
      setErro(val.error.issues.map((i) => i.message).join(' · '))
      return
    }
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API_URL}/auth/finalizar-cadastro`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(val.data),
      })
      ...
      if (!res.ok) {
        setErro(data.detail ?? 'Erro ao finalizar cadastro.')
        return
      }
  ...
  async function handleReenviar() {
    ...
    try {
      ...
      // silencioso — evita revelar estado do servidor
    }
  }
